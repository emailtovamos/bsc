...
type BlobSaver interface {
	SaveBlobSidecar(context.Context, *params.ChainConfig, []*types.BlobTxSidecar) error
}

type BlobGetter interface {
	GetBlobSidecarsByRoot(context.Context, [32]byte) ([]*types.BlobTxSidecar, error)
	GetBlobSidecarsBySlot(context.Context, *params.ChainConfig, primitives.Slot) ([]*types.BlobTxSidecar, error)
}
...
type BlobCrud interface {
	BlobSaver
	BlobGetter
	BlobDeleter
}

func SaveBlobSidecar(ctx context.Context, saver BlobSaver, config *params.ChainConfig, scs []*types.BlobTxSidecar) error {
	sortSideCars(scs)
...
func GetBlobSidecarsByRoot(ctx context.Context, getter BlobGetter, root [32]byte, indices ...uint64) ([]*types.BlobTxSidecar, error) {
...
func GetBlobSidecarsBySlot(ctx context.Context, getter BlobGetter, config *params.ChainConfig, slot primitives.Slot, indices ...uint64) ([]*types.BlobTxSidecar, error) {
...
func filterForIndices(scs []*types.BlobTxSidecar, indices ...uint64) ([]*types.BlobTxSidecar, error) {
	if len(indices) == 0 {
		return scs, nil
	}
	...
	sidecars := make([]*types.BlobTxSidecar, len(indices))
...
func verifySideCars(scs []*types.BlobTxSidecar) error {
	if len(scs) == 0 {
	...
	for i, sc := range scs {
		if sc.BlockSlot != sl {
			return fmt.Errorf("sidecar slot mismatch: %d != %d", sc.BlockSlot, sl)
		}
...
func sortSideCars(scs []*types.BlobTxSidecar) {
