...
func (blob *Storage) SaveBlobSidecar(config *params.ChainConfig, scs []*types.BlobTxSidecar) error {
...
func (blob *Storage) GetBlobSidecarsByRoot(root [32]byte) ([]*types.BlobTxSidecar, error) {
	var enc []byte
...
func (blob *Storage) GetBlobSidecarsBySlot(config *params.ChainConfig, slot primitives.Slot) ([]*types.BlobTxSidecar, error) {
	var enc []byte
...
func blobSidecarKey(blob *types.BlobTxSidecar, config *params.ChainConfig) []byte {
	key := slotKey(blob.BlockSlot, config)
