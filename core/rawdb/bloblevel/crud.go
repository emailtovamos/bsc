...
// todo 4844 Satyajit Ensure you can load pre-existing db instead of creating a new one each time there is restart
...
func (blob *Storage) SaveBlobSidecar(ctx context.Context, config *params.ChainConfig, scs []*types.BlobTxSidecar) error {
	slot := scs[0].BlockSlot
...
func (blob *Storage) GetBlobSidecarsByRoot(ctx context.Context, root [32]byte) ([]*types.BlobTxSidecar, error) {
	var enc []byte
	...
	scs := make([]*types.BlobTxSidecar, 0)
...
func (blob *Storage) RetrieveSidecar(root common.Hash) *types.BlobTxSidecar {
...
func (blob *Storage) GetBlobSidecarsBySlot(ctx context.Context, config *params.ChainConfig, slot primitives.Slot) ([]*types.BlobTxSidecar, error) {
	var enc []byte
...
func blobSidecarKey(blob *types.BlobTxSidecar, config *params.ChainConfig) []byte {
	key := append(blobSidecarPrefix, slotKey(blob.BlockSlot, config)...)
	key = append(key, bytesutil.Uint64ToBytesBigEndian(uint64(blob.BlockSlot))...)
